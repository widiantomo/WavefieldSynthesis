...
kec = str2double(get(handles.kecSuara,'String'));
set(handles.SourceDir,'UserData',[])
...
% set aliasing
frekal = kec / (2*Spasi);
set(handles.infoalias,'String',num2str(frekal));
...
poslist_x = str2double(get(handles.listx,'String'));
poslist_y = str2double(get(handles.listy,'String'));;
...
cd('C:\MATLAB71\work\WaFS\GUIfigure\Dry Source\Music');
...
% -----------------------------------------------------------------------%
% --- Virtual source selection on axes-----------------------------------%
% -----------------------------------------------------------------------%
% --- Room model --------------------------------------------------------%
% -----------------------------------------------------------------------%
...
lixy = zeros(1,2);
lixy(1) = str2num(get(handles.listx,'String'));
lixy(2) = str2num(get(handles.listy,'String'));

rrr = sqrt(((lixy(2) - currPt(2))^2)+((lixy(1) - currPt(1))^2));

ff = num2str(rrr);
fl = num2str(currPt);
butt = questdlg(['Do you really want to put it here ?   ' fl ' :distance = ' ff],'Question!','Yes','No','InsertPoint','Yes');
...
ggg = 'Yes';
ttt = 'InsertPoint';
if strcmp(ttt,butt) == 1
    pointx = (inputdlg('insert coordinate x','virtual source'));
    pointy = (inputdlg('insert coordinate y','virtual source'));
    currPt = [str2num(pointx{1}) ; str2num(pointy{1})];
    currPt = currPt';
end
    
if ((strcmp(ggg,butt) == 1)|strcmp(ttt,butt) == 1)  & size(ptList,1)<val
    axes(handles.axes1);
     hhhh=[]
     numm = 0;
     figNumber=gcf;
     ...
     while size(ptList,1)<val
        ...
        set(handles.checkbox4, 'UserData', PathName);
...
elseif size(ptList,1)>=val
disp('User selection exceeded')
else
ptList = []
end
...
%get source position
 sr = get(gca,'UserData');   % Source position [x y z] (m) untuk manual
% ganti ini
...
function listx_Callback(hObject, eventdata, handles)
% hObject    handle to listx (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of listx as text
%        str2double(get(hObject,'String')) returns contents of listx as a double


% --- Executes during object creation, after setting all properties.
function listx_CreateFcn(hObject, eventdata, handles)
% hObject    handle to listx (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function listy_Callback(hObject, eventdata, handles)
% hObject    handle to listy (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of listy as text
%        str2double(get(hObject,'String')) returns contents of listy as a double


% --- Executes during object creation, after setting all properties.
function listy_CreateFcn(hObject, eventdata, handles)
% hObject    handle to listy (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Otherwise, executes on mouse press in 5 pixel border or over checkbox3.
...
% --- Executes on button press in stereo reproduction.
...
if (get(hObject,'Value') == get(hObject,'Max'))
set(handles.Jumlah,'String',2);
set(handles.dl,'String',1);
else
set(handles.Jumlah,'String',5);
set(handles.dl,'String',1);
end
