...
%  weighting window is the cosine square (one of my favorites).  Commnents
%  can be changed inside code to activate a different windowing options.
...
%  WINDOW WEIGHT OPTIONS .....
    ...
    %  so your desired window FIR fitler is uncommented
...
%       h = (0:1)-k/(n+1);
%       h = abs(h);
    h = h/sqrt(sum(h.^2));  % Normalized filter coefficients  (comment out for linear interpolation, in for all others)
